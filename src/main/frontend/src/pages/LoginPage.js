    ...
    <div className="h-screen w-screen bg-[url('./images/login-bg.jpg')] bg-cover">
      <div className="login-wrap">
        <div className="login form absolute top-1/3 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
          ...
          <img src={logo} alt="title_logo" className="w-80 h-52"/>
          </div>
          {error ?(<div>{error}</div>):('')}
          <form className="max-w-sm mx-auto w-full items-center" onSubmit={loginSubmit}>
            <div className="flex flex-col pt-4">
              ...
              <input type="text"  className="border-none mb-1 rounded-sm h-10 w-80" onChange={e => setUserId(e.target.value)} value={userId}/>
              <label htmlFor="userPw" className="text-white">Password</label>
              <div className="relative">
                <input type={showPassword ? "text" : "password"}  className="border-none pr-48 rounded-sm h-10 w-80" onChange={e => setUserPw(e.target.value)} value={userPw}/>
              ...
              <button type="submit" className="text-lg leading-4 font-medium bg-blue-500 hover:bg-sky-700 h-12 mt-16 text-white rounded-sm" onClick={loginSubmit}>로그인</button>
            </div>
          </form>
        </div>
        <div className=""></div>
